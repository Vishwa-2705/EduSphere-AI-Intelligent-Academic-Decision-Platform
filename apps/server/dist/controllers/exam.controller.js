...
exports.getMyReportCard = exports.submitExamGrades = exports.getExamGrades = exports.createExam = exports.getExamSchedule = void 0;
...
const getExamGrades = async (req, res, next) => {
    try {
        ...
        const exam = await Exam_1.Exam.findById(examId).populate('course');
        if (!exam) {
            res.status(404).json({ success: false, message: 'Exam not found' });
            return;
        }
        const enrollments = await Enrollment_1.Enrollment.find({ course: exam.course, status: 'ENROLLED' }).populate('student');
        const existingGrades = await Exam_1.GradeEntry.find({ exam: examId });
...
const submitExamGrades = async (req, res, next) => {
    try {
        ...
        const exam = await Exam_1.Exam.findById(examId);
        if (!exam) {
            res.status(404).json({ success: false, message: 'Exam not found' });
            return;
        }
        const computeGradeLetter = (pct) => {
            if (pct >= 90)
                return 'O';
            if (pct >= 80)
                return 'A+';
        ...
        await Promise.all(grades.map(async (g) => {
            ...
            await Exam_1.GradeEntry.findOneAndUpdate({ exam: exam._id, student: g.studentId }, {
                course: exam.course,
...
exports.submitExamGrades = submitExamGrades;
const getMyReportCard = async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        const profile = await Profile_1.Profile.findOne({ user: userId }).populate('department');
        const enrollments = await Enrollment_1.Enrollment.find({ student: userId, status: 'ENROLLED' }).populate('course');
        const grades = await Exam_1.GradeEntry.find({ student: userId }).populate('exam').populate('course');
        ...
        const courseGradeSummary = enrollments.map((en) => {
            ...
            return {
                ...
                examBreakdown: courseGrades.map((cg) => ({
...
exports.getMyReportCard = getMyReportCard;
