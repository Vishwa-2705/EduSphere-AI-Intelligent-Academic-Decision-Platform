...
export const getStudentRiskPrediction = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const grades = await GradeEntry.find({ student: userId });
    ...
    try {
      const aiResponse = await axios.post('http://127.0.0.1:8000/predict-risk', featurePayload, { timeout: 2000 });
