...
export const getStudentTimetable = async (
  ...
  try {
    ...
    const slots = await TimetableSlot.find({ semester, section })
      .populate('course')
      .populate('department')
    ...
    for (const d of days) {
      ...
      weeklySchedule[d] = daySlots.map((s: any) => ({
        id: s._id,
...
export const getFacultyTimetable = async (
  ...
  try {
    ...
    const slots = await TimetableSlot.find({ faculty: facultyId })
      .populate('course')
      .populate('department')
...
export const runOptimizationSolver = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const dept = await Department.findById(departmentId) || await Department.findOne();
    const courses = await Course.find({ semester: semester || 6 });
