...
const getStudentFeeConfig = (profile: any) => {
...
export const getMyFeeInvoice = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    let invoice = await FeeInvoice.findOne({ student: userId }).sort({ createdAt: -1 });
...
export const payFeeOnline = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const invoice = await FeeInvoice.findOne({ student: userId });
    if (!invoice) {
...
export const getAdminFeeOverview = async (
  _req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const invoices = await FeeInvoice.find().populate('student');
